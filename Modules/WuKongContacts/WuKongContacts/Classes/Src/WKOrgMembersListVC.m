...
@interface WKOrgMembersListVC () <UITableViewDataSource, UITableViewDelegate>

@property(nonatomic,strong) UITableView *tableView;
@property(nonatomic,strong) WKOrgMembersListVM *vm;
@property(nonatomic,strong) NSMutableArray *sectionTitleArr;
@property(nonatomic,strong) NSMutableArray<NSMutableArray*> *items;

@end

@implementation WKOrgMembersListVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationBar.title = LLang(@"组织内联系人");
    self.vm = [WKOrgMembersListVM new];
...
- (void)requestData {
    ...
    [self.vm requestMembers].then(^(NSArray<WKOrgMemberResp*>* members) {
        NSMutableArray *cellModels = [NSMutableArray array];
        for (WKOrgMemberResp *member in members) {
            WKContactsCellModel *model = [WKContactsCellModel new];
            model.uid = member.uid;
            model.name = member.name;
            model.avatar = [WKAvatarUtil getAvatar:member.uid];
            [cellModels addObject:model];
        }
        [weakSelf sortAndGroup:cellModels];
    }).catch(^(NSError *error) {
        NSLog(@"WKOrgMembersListVC requestData error: %@", error);
    });
}
