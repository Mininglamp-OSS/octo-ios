...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if ([text isEqualToString:@""] && range.length == 1) {
        [self maybeRemoveMentionAtCursor:textView range:range];
    }
    return YES;
}
...
- (void)maybeRemoveMentionAtCursor:(UITextView *)tv range:(NSRange)range {
    NSString *text = tv.text ?: @"";
    if (range.location >= text.length) return;
    NSString *willDel = [text substringWithRange:range];
    if (![willDel isEqualToString:WKInputAtEndChar]) return;
