...
@objc public final class WKRocketSmokeCloud: UIView {
    ...
    /// 强度 → 烟雾颜色映射（始终保持"浅灰白"范围，不会出现黑烟）
    ///   0   ~ 0.7 : 纯白 0.97 → 近白 0.94   （点火初期纯水蒸气）
    ///   0.7 ~ 1.4 : 近白 0.94 → 浅灰 0.88   （推力加大轻微变灰）
    ///   1.4 ~ 2.0 : 浅灰 0.88 → 浅灰 0.82   （峰值也只到浅灰，不会更深）
    ///
    /// 整体颜色全程在 0.82 ~ 0.97 区间 → 始终是"淡淡的灰白烟"，
    /// 不会让人觉得"发动机故障冒黑烟"。
    private static func smokeColorForIntensity(_ intensity: CGFloat) -> UIColor {
        let i = max(0.0, min(2.0, intensity))
        if i < 0.7 {
            let t = i / 0.7
            let w = 0.97 - 0.03 * t         // 0.97 → 0.94
            return UIColor(white: w, alpha: 1.0)
        } else if i < 1.4 {
            let t = (i - 0.7) / 0.7
            let w = 0.94 - 0.06 * t         // 0.94 → 0.88，极轻微微蓝冷调
            return UIColor(red: w - 0.008 * t,
                           green: w - 0.004 * t,
                           blue: w + 0.006 * t,
                           alpha: 1.0)
        } else {
            let t = min((i - 1.4) / 0.6, 1.0)
            let w = 0.88 - 0.06 * t         // 0.88 → 0.82（峰值最深也只是浅灰）
            return UIColor(red: w - 0.010 * t,
                           green: w - 0.005 * t,
                           blue: w + 0.008 * t,
                           alpha: 1.0)
        }
