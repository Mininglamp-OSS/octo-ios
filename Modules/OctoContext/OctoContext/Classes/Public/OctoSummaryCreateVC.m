...
- (void)loadTemplates {
    ...
    [[OctoSummaryAPI shared] getTopicTemplates:^(id _Nullable result, NSError * _Nullable error) {
        ...
        for (OctoTopicTemplate *t in arr) {
            [localized addObject:[weakSelf localizeKnownTemplate:t]];
        }
        weakSelf.templates = localized;
        [weakSelf renderChips];
    }];
}
...
- (void)onPickSource {
    ...
    for (OctoSourceItem *s in self.selectedSources) {
        NSInteger ct = (s.sourceType == OctoSourceDirectMessage) ? WK_PERSON : WK_GROUP;
