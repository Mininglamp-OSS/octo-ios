...
> 下列模块名将在开源准备 P6 / P7 阶段统一迁移到 `Octo*` 前缀，目前为过渡命名。

```
.
├── TangSengDaoDao/          # 主 App target（迁移中 → Octo）
...
- [ ] P4 子模块 podspec 规范化
- [ ] P5 不兼容许可证代码替换
- [ ] P6 主工程命名迁移（`TangSengDaoDao` → `Octo`）
- [ ] P7 模块命名迁移（`WuKong*` → `Octo*`）
- [ ] P8 仓库历史清理
